...
-- neu (optional statt harter Fehler):
...
local function CheckBuff(unit)
    ...
    local i = 1
    while true do
        local aura = C_UnitAuras.GetBuffDataByIndex(unit, i, "PLAYER")
        if not aura then break end

        if aura.sourceUnit == "player" and isTracked(aura.spellId) then
            return (aura.expirationTime and (aura.expirationTime - GetTime())) or 0, aura.spellId
        end
        i = i + 1
    end
end
...
local function updateHealth(health, spellID)
	...
	if not health.BuffHighlightActive and not health.BuffHighlightFaderActive then
		return
	end

	local t = E.db.unitframe.colors
	-- Get highlight color for the spell
	if health.BuffHighlightActive then
		t = E.db.BH.spells[spellID].glowColor
	elseif health.BuffHighlightFaderActive then
		t = E.db.BH.spells[spellID].fadeColor
	end
...
function BH:Initialize()
	-- Sicherstellen, dass die DB existiert (falls Addon nachträglich geladen wird)
	if not E.db.BH then
		E.db.BH = {
			enable = true,
			colorBackdrop = false,
			...
			spells = {},
			refreshRate = 0.1,
			trackedHeaders = { party = true, raid1 = true, raid2 = true, raid3 = true, player = false, target = false },
		}
	end
	...
	-- neu (Callback, und ohne Lib fallback direkt einhängen):
	if EP then
		EP:RegisterPlugin(addonName, function() BH:InsertOptions() end)
	else
		BH:InsertOptions()
	end
end
