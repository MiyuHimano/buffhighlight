...
-- Retail API: GetSpellInfo ist jetzt C_Spell.GetSpellInfo
...
function BH:InsertOptions()
	local version = format('|cff1784d1v%s|r', C_AddOns.GetAddOnMetadata(addonName, 'Version') or "unknown")
	...
	E.Options.args.BH = {
		...
		args = {
			title = {
				order = 1,
				type = "header",
				name = format('|cff00b3ffBuffHighlight|r [%s] by |cfff48cbaKaalos|r (|cffff0000Retail Fix|r)', version),
			},
			addonOptions = {
				order = 2,
				type = "group",
				...
				args = {
					addSpell = {
						...
						set = function(info, value)
							...
							local spellName = SafeGetSpellInfo(value)
					...
					selectSpell = {
						...
						values = function()
							...
							for id, spell in pairs(list) do
								local spellName = tonumber(id) and SafeGetSpellInfo(id)
			...
			spellGroup = {
				...
				name = function()
					local spell = GetSelectedSpell()
					local spellName = spell and SafeGetSpellInfo(spell)
